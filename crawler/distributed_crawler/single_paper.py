...
def crawlAllMainPaper():
    interface = MongoDBInterface()
    interface.setCollection(main_paper_with_abstract)
    for doc in interface.getAllDocuments(sorting_info=[('_id', 1)]):
        if 'meta' in doc.keys():
            continue
        res = crawlSinglePaper(doc['_id'])
        doc['meta'] = res
        interface.updateDocument(doc)
        #pprint(res)
        a = random.randint(1, 100) % 2 + 1
        time.sleep(a)

def test():
    print crawlSinglePaper(47833789)
